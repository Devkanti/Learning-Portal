...
const TeacherDashboard = () => {
  ...
  // --- NEW: Calculate today's date in YYYY-MM-DD format to restrict the calendar ---
      ...
      <main className="main-content teacher-layout">
              ...
              <form onSubmit={handleAddLecture}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px'}}>
                  ...
                  <div className="form-group">
                    ...
                    {/* FIX: Added min={todayFormatted} to prevent past dates */}
                    <input type="date" min={todayFormatted} value={newLectureDeadline} onChange={(e) => setNewLectureDeadline(e.target.value)} />
                  </div>
                </div>
        ...
        {activeTab === 'create' && (
           ...
           <form onSubmit={handleCreateCourse} className="mt-4">
             ...
             <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px', backgroundColor: 'var(--primary-light)', padding: '24px', borderRadius: 'var(--radius-lg)', borderLeft: '4px solid var(--primary)', marginBottom: '24px'}}>
              <div className="form-group"><label style={{ color: 'var(--primary-hover)' }}>Attach Assessment Worksheet (Word or PDF)</label><input type="file" accept=".pdf,.doc,.docx" onChange={(e) => setAssignmentFile(e.target.files[0])} /></div>
              <div className="form-group">
                <label style={{ color: 'var(--primary-hover)' }}>Submission Deadline</label>
                {/* FIX: Added min={todayFormatted} to prevent past dates */}
                <input type="date" min={todayFormatted} value={deadline} onChange={(e) => setDeadline(e.target.value)} />
              </div>
             </div>
